    ...
    <Shell>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map(item => <ContentRow key={item.id} item={item} username={username} />)}
        </div>
      )}
    </Shell>
  );
...
function ContentRow({ item, username }: { item: any; username: string }) {
  const pub = usePublicClient();
  const { data: wallet } = useWalletClient();
  const [editing, setEditing] = useState(false);
  const [newPrice, setNewPrice] = useState('');
  const [newActive, setNewActive] = useState(item.active);
  const [busy, setBusy] = useState(false);
  ...
  const save = async () => {
    ...
    setBusy(true); setErr('');
    try {
      // updateContent(username, contentId, active, newPrice, newMetadata)
      const price = newPrice ? parseUnits(newPrice, 18) : 0n;
      const hash = await wallet.writeContract({
        address: ADDRESSES.contentPaywall, abi: contentPaywallFullAbi, functionName: 'updateContent',
        args: [username, item.id, newActive, price, ''],
      });
      await pub.waitForTransactionReceipt({ hash });
      setEditing(false);
    ...
    <div className="bg-panel border border-border rounded-2xl p-5">
      <div className="flex justify-between items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs mt-1">{item.active ? '🟢 active' : '⚪ inactive'}</div>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button onClick={() => { setEditing(!editing); setNewActive(item.active); setNewPrice(''); setErr(''); }}
        ...
        <div className="mt-4 pt-4 border-t border-border space-y-3">
          <div className="flex items-center gap-3">
            ...
            <input value={newPrice} onChange={e => setNewPrice(e.target.value.replace(/[^0-9.]/g, ''))}
              placeholder={formatUnits(item.price, 18)}
          ...
          <div className="text-[11px] text-muted">
            Leave price blank to keep current. Existing buyers retain lifetime access.
          </div>
        </div>
      )}
    </div>
